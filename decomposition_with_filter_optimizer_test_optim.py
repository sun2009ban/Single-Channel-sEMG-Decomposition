...
EMG = np.load('./EMG_for_decomposition/iEMG.npy')
EMG = np.squeeze(EMG)
EMG = EMG[1000:1000 + 512]
...
G_DC.load_state_dict(torch.load('./saveTorch/G_DC_94_6000.pth'))
G_DC = G_DC.to(DEVICE, DTYPE)
G_DC.eval()

D_DC = Model.build_dc_classifier(ndf=NDF)
D_DC.load_state_dict(torch.load('./saveTorch/D_DC_94_6000.pth'))
