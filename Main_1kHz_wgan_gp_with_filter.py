...
if RESTORE_PREVIOUS_MODEL:
    D_DC.load_state_dict(torch.load(os.path.join(SAVE_DIR, 'D_DC_0_0.pth'), map_location='cuda'))
    G_DC.load_state_dict(torch.load(os.path.join(SAVE_DIR, 'G_DC_0_0.pth'), map_location='cuda'))

if torch.cuda.device_count() > 1:
    print('Parallel in {} GPU'.format(torch.cuda.device_count()))
    D_DC = nn.DataParallel(D_DC)
    G_DC = nn.DataParallel(G_DC)
