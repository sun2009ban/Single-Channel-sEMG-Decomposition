        ...
        for ii, x in enumerate(train_data):
            ...
            if ii % save_every == 0:             
                ...
                if torch.cuda.device_count() > 1:
                    torch.save(D_net.module.state_dict(), os.path.join(save_dir, 'D_DC_' + str(epoch % maximum_save_size) + '_' + str(ii) + '.pth'))
                    torch.save(G_net.module.state_dict(), os.path.join(save_dir, 'G_DC_' + str(epoch % maximum_save_size) + '_' + str(ii) + '.pth'))                   
                else:
